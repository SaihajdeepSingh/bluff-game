...
const { startGame, validatePlay, playCards, resolveBluff, passTurn } = require('./gameEngine');
  ...
  socket.on('call-bluff', ({ code }) => {
    try {
      ...
      room.players.forEach(p => io.to(p.id).emit('bluff-result', {
        ...
        nextPlayerNickname:  result.nextPlayerNickname,
  ...
  socket.on('pass-turn', ({ code }) => {
    try {
      ...
      if (room.currentTurn !== socket.id) {
        socket.emit('play-error', { error: 'Not your turn' });
        return;
      }
      ...
      if (result.flushed) {
        io.to(code).emit('pile-flushed', {
          message: 'All players passed — pile discarded!',
        });
      } else {
        io.to(code).emit('player-passed', {
          nickname:    passer.nickname,
          passCount:   room.passCount,
          totalPlayers: connected,
        });
      }
    } catch(e) { console.error('pass-turn', e.message); }
  });
