...
export default function GameScreen({ room, setRoom, nickname, roomCode, onLeave }) {
  ...
  const [pending,  setPending]  = useState(null); // pending win banner
  ...
  useEffect(() => {
    const onUpdate  = ({ room: r }) => { setRoom(r); setSelected([]); };

    const onPending = ({ nickname: n }) => {
      setPending(n);
    };

    const onBluff = ({ isBluff, actualCards, loserNickname, claimerNickname, challengerNickname, room: r }) => {
      setRoom(r); setSelected([]);
      setPending(null);
      setReveal(actualCards);
      flash(
        isBluff
          ? `Bluff caught! ${claimerNickname} lied — ${loserNickname} takes the pile. ${challengerNickname} plays next.`
          : `No bluff! ${claimerNickname} was honest — ${challengerNickname} takes the pile and plays next.`,
        isBluff ? 'bluff' : 'safe'
      );
      setTimeout(() => setReveal(null), 3500);
    };

    const onOver = ({ winner }) => { setPending(null); setOver(winner); };
  ...
  function toggle(card) {
    ...
    setSelected(prev => {
      ...
      if (has) return prev.filter(c => c.rank + c.suit !== key);
  ...
  return (
    <div className="game-root">
      ...
      <div className="topbar">
        ...
        <button className="tb-btn" onClick={onLeave} title="Leave"><LogOut size={14} /></button>
      </div>
            ...
            <div className="last-play" style={{ flexDirection:'column', gap:'.38rem' }}>
              ...
              <div className="reveal-row">
                {reveal.map((c, i) => {
                  ...
                  return (
                    <div key={i} className={`pc ${r ? 'pc-red':'pc-black'}`} style={{ cursor:'default', animation:'none' }}>
      ...
      <div className="actions">
        ...
        {canBluff && (
          <button className="btn btn-red btn-pill" onClick={() => socket.emit('call-bluff', { code: roomCode })}>
        ...
        <div className="overlay" onClick={() => setModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="rank-grid">
              {RANKS.map(r => (
                <button key={r} className={`rank-btn${rank === r ? ' chosen' : ''}`} onClick={() => setRank(r)}>
                  {r}
                </button>
              ))}
            </div>
