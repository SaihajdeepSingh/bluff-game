...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onGameStarted    = ({ room }) => { setRoom(room); setScreen('game'); };
  ...
  if (rejoining) return (
    <div className="reco">
      ...
      <p style={{ color:'var(--t3)', fontSize:'.86rem' }}>Rejoining your game...</p>
      <button onClick={() => { ls('clear'); setRejoining(false); }}
        style={{ background:'none', border:'none', color:'var(--t4)', cursor:'pointer', fontSize:'.75rem', marginTop:'.2rem' }}>
        Cancel
      </button>
    </div>
  );
    ...
    <div className="screen">
      <div className="lobby-wrap">
        ...
        <button className="btn btn-ghost"
          onClick={() => { ls('clear'); setScreen('home'); setRoom(null); setCode(''); }}>
          Leave Room
        </button>
      </div>
    </div>
